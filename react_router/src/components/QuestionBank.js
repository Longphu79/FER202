...
function QuestionBank() {
  ...
  useEffect(() => {
    if (showScore) return; // 🛑 Dừng timer khi quiz kết thúc
  ...
  }, [timeLeft, showScore]); // ✅ Thêm showScore vào dependency để tránh lỗi khi quiz kết thúc
          ...
          <div>
            ...
            <div className="d-grid gap-2 mt-3">
              {questions[currentQuestion].options.map((option, index) => (
                <Button
                  key={index}
                  variant={selectedOption === option ? "success" : "outline-dark"}
